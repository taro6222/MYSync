...
동기화 목록의 설정에서 폴더 변경, 검사·실행, 충돌·복구, 연결 삭제를 처리합니다. 동기화 추가는 팝업으로 열립니다. [변경 동작과 EXE 시작 성능 개선 제안](docs/pair-settings-and-startup.md)을 확인하세요.

# MYSync

## 전송 성능 업데이트 (2026-09-08)

파일별 전송·검증 완료를 즉시 표시하고 전체 비교는 별도로 진행한다. Google 체크섬·버전별 검증 결과 재사용, 병렬 검사, 자동 실행의 중복 전체 검사 제거, 스트리밍 해시, 로그 정리 비용 감소를 적용했다. [성능 동작·검증](docs/transfer-performance.md)을 참고한다.


## 파일 오류 해결 업데이트 (2026-09-08)

전송 화면에 오류 해결·이번만 제외를 추가했다. 해결 창에서 재검사, 방향 선택 후 선택 파일 전송, 원본 사본 보존, 이번 실행 제외를 제공한다. [사용 안내](docs/file-error-actions.md)를 참고한다.


## 사이드바·동기화 지속성 업데이트 (2026-09-08)

전체 메뉴를 사이드바로 통합했다. Google ETag 호환 덮어쓰기, 최대 3개 파일 병렬 전송, 정상 파일별 비교 기준 저장, 오류 이후 새 변경 처리와 구버전 Applied 기록 복원을 추가했다. [동작 및 검증](docs/sidebar-and-concurrent-sync.md)을 참고한다. 아래 과거의 순차 실행·오류 시 확인 대기 고정 설명보다 이 업데이트를 우선한다.


## 대용량·파일 제어·설정 개편 (2026-09-08)

Google 파일 크기 제한을 제거하고 대용량 청크 업로드를 추가했다. 진행·대기 파일의 중지/취소/재개/제외, 실행파일 옆 ,log 폴더의 용량 제한 순환 로그, 카테고리별 설정 UI를 제공한다. [동작·제한·검증 안내](docs/large-transfers-and-logs.md)가 아래 과거 5 MiB 제한 및 일자별 로그 설명보다 우선한다.


## 오류 알림·개별 정책·OAuth 배포 업데이트 (2026-09-08)

트레이 오류 알림 클릭 시 해당 오류 페이지로 이동한다. 연결별 파일·폴더 제외와 합산 전송 속도(KiB/s, 0=무제한)를 설정할 수 있다. Google 설정은 DLL 내부 리소스로 전환해 배포 폴더에 oauth-client.json을 복사하지 않는다. 내장 리소스는 추출 가능하므로 비밀 보장으로 간주하지 않는다. [설정 및 테스트](docs/sync-options-and-alerts.md), [OAuth 배포와 보안 한계](docs/oauth-distribution.md)를 참고한다. 아래 구버전 설명과 다르면 이 업데이트를 우선한다.
