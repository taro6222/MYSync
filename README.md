...
현재 **WebDAV 연결과 실제 파일 전송, 자동 동기화, 트레이 실행**을 구현했습니다. Google Drive는 OAuth 연결·폴더 탐색 단계이며 전송은 미구현입니다. 실제 서버와 Windows UI에 대한 수동 검증은 남아 있습니다.
...
| Google Drive | OAuth 로그인·암호화 저장·내 드라이브 폴더 탐색 (전송 미구현) |
