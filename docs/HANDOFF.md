...
**자동 검증 실행 여부에 주의하세요.** 충돌·복구 확인 화면까지는 전체 자동 검증을 실행해 통과를 확인했습니다. 그 이후 변경 — 전송 진행률·계정 관리, 오류 분류·재시도·검사 캐시, 미지원 항목 분류 — 은 **빌드만 확인했고 자동 검증을 실행하지 않았습니다.** 해당 검증 코드(`ManagementChecks`, `ResilienceChecks`, `UnsupportedChecks`)도 아직 한 번도 실행되지 않았습니다. 다음 작업 전에 `build.ps1 -Check`를 먼저 돌리세요.
...
0. `build.ps1 -Check`를 실행해 밀린 자동 검증을 통과시킵니다. 실패하면 그것부터 고칩니다.
1. **로깅을 추가합니다.** 파일 로그와 함께 작업별 실패 사유·분류를 큐에 저장합니다. WebDAV 응답 상태와 ETag 유무도 남깁니다. 이게 없으면 다음 실서버 문제도 추측으로 풀게 됩니다.
