...
- 진단 기능은 `3f50c3c`로 main에 커밋·푸시했다.
- PROPFIND Depth:0의 강한 getetag → If-Match GET 경로를 추가했다. GET 헤더가 없는 경우에도 검증된 버전으로 교체·삭제·임시 파일 정리를 수행한다.
- 모의 서버에서 속성 ETag 전용 왕복 및 동시 변경 거부를 검증했고 전체 자동 검증이 통과했다. [ETag 보완](webdav-etag.md) 참고.
- 실서버 재검증은 남아 있다. 강한 ETag를 어디에서도 얻지 못한 경우의 조건 없는 삭제는 구현하지 않았다. 기존 NAS 잔류 임시 파일도 건드리지 않았다.
- 다음은 새 빌드로 Synology getetag 제공과 조건부 요청 집행을 확인하는 작업이다. 이전 실행의 임시 파일 관리 UI는 후속 범위다.
...
**현재 단계: Synology 1차 전송 실패를 바탕으로 진단 로그와 ETag 획득 경로를 보완했고, 실서버 재검증을 기다립니다.** 아래 ‘실서버 1차 테스트 결과’는 수정 전 기록입니다. 기능 구현은 충돌·복구 확인 화면, 전송 진행률, 계정 관리, 오류 분류·재시도, 검사 캐시, 미지원 항목 분류까지 마쳤습니다.
