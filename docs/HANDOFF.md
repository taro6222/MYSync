...
- 설정에서 로컬/원격 폴더를 변경하고 검사·실행, 충돌·복구, 연결 삭제를 실행합니다.
- 동기화 추가는 목록 버튼의 모달 팝업으로 이동했습니다.
- 루트/계정 변경은 새 ID로 원자적 교체하여 이전 대기열 적용을 방지하고 일시정지로 저장합니다.
- EXE 배포 방식은 유지. 설치형/framework-dependent 전환 제안 및 수동 확인 항목: [상세](pair-settings-and-startup.md).
