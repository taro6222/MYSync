...
public sealed class SettingsStore
{
    ...
    public void Save(SyncPair pair)
    {
        using var connection = Open();
        using var command = connection.CreateCommand();
        command.CommandText = "INSERT INTO SyncPairs VALUES ($id,$provider,$local,$remote,$name,$paused) ON CONFLICT(Id) DO UPDATE SET ProviderId=$provider,LocalPath=$local,RemoteFolderId=$remote,RemoteFolderName=$name,Paused=$paused";
    ...
    public IReadOnlyList<SyncPair> Load()
    {
        using var connection = Open();
        using var command = connection.CreateCommand();
        command.CommandText = "SELECT Id,ProviderId,LocalPath,RemoteFolderId,RemoteFolderName,Paused FROM SyncPairs ORDER BY rowid";
        using var reader = command.ExecuteReader();
        var result = new List<SyncPair>();
        while (reader.Read()) result.Add(new(Guid.Parse(reader.GetString(0)), reader.GetString(1), reader.GetString(2), reader.GetString(3), reader.GetString(4), reader.GetBoolean(5)));
        return result;
    }
}
